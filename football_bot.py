...
alwd_group = [-1001837008506] if ENV == 'UAT' else [-1001536861033]
grpID = -1001837008506 if ENV == 'UAT' else -1001536861033  # чат для записи и болталка
...
def games_extract(message, desc_input_to_games):
    ...
    appl_list_main_msg = '<b>🗓' + format_date1(dateID) + ' 🕖 ' + timeID + '⏳' + dur + 'мин </b>\n \n📍 ' + locationID + '\n&#128176; Стоимость: ' + str(priceID) + '\n&#128179; Оплата только безнал\n&#9997; Запись/отмена через бота\n \n' + players_list + '\n \n-----------\n <b>Резерв</b> \n' + res_pl_list + '\n \n ----------\nвсе вопросы ' + bot_name_val + '\n\n#УВАЖЕНИЕ'
...
but1_text = 'ℹ Общая информация о тренировках'
...
repl_txt4 = None
repl_txt6 = None
repl_err1 = 'Тренировка уже в процессе. Запись на следующую тренировку будет открыта завтра'
...
keyboard1 = ReplyKeyboardMarkup(resize_keyboard=True, row_width=2).add(*[KeyboardButton(text=text) for text in [but1_text, but2_text, but4_text, but6_text]]).add(KeyboardButton(text=but7_text))
keybrd = keyboard1
# create the custom keyboard1

# create the custom keyboard2
keyboard2 = ReplyKeyboardMarkup(resize_keyboard=True, row_width=2)
buttons2 = [KeyboardButton(text=text) for text in [but8_text, but9_text]]
